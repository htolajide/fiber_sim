    ...
    def generate_input_mac(self):
        try:
            ...
            with open(mac_path, 'w') as f:
                ...
                # GPS Configuration
                f.write("# Source Setup (General Particle Source) GPS\n")
                f.write("/gps/pos/type Point\n")
                f.write("/gps/pos/centre 0 0 -30 mm\n")
    ...
    def analyze_dose(self):
        ...
        try:
            ...
            for i in range(self.layer_table.rowCount()):
                ...
                name = w(0).text().strip()
                mat_name = w(1).currentText().strip()
                layer_type = w(2).currentText() if isinstance(w(2), QComboBox) else "Hollow Cylinder"
                ir_um = float(w(3).text()) if w(3).isEnabled() else 0.0
                ...
                if hasattr(self, 'layer_table'):
                    ...
                    for i in range(self.layer_table.rowCount()):
                        ...
                        geom_data.append({
                            ...
                            'Inner Radius (μm)': w(2).text(),
                            'Outer Radius (μm)': w(3).text(),
                            'Length (mm)': w(4).text()
                        })
